...
/**
 * GitHub project Pages serves this app under `/core/`. React Router must use the same basename or
 * visitor routes resolve the path segment `core` as a page slug → in-app 404. Injects `basename`
 * on `BrowserRouter` in @olonjs/core at bundle time only (no edits to packages/core on disk).
 */
function ghPagesBrowserRouterBasename(): Plugin {
  return {
    name: 'olonjs-gh-pages-browser-router-basename',
    enforce: 'pre',
    transform(code, id) {
      const n = id.replace(/\\/g, '/');
      if (!n.includes('packages/core/src/lib/JsonPagesEngine.tsx')) return null;
      if (code.includes('<BrowserRouter basename')) return null;
